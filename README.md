...
Static **Pareto** UI: `artifacts/pareto_viewer/index.html` — load `pareto_front.json` (from `scripts/export_pareto_front.py`); bundled `sample_pareto_front.json`; HTTP **Presets** via `local_presets.json`; hover / click / arrow keys to inspect archive indices.
