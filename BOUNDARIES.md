...
**Moonshot (ensemble dispersion):** `fragility_engine.benchmarks.ensemble` + `scripts/fragility_robustness_sweep.py` — same genome and rollout seed; sweep **`graph_seed`** **or** bundle **`--neighbor-json-list`** (`robustness_rollouts_neighbor_json_bundle`); emit quantiles (**not** within-rollout stochasticity). **1D / 2D scans:** `robustness_ensemble_1d_param_sweep`, `robustness_ensemble_2d_param_grid` (+ optional **`neighbor_weights_json_paths`**); schemas **`fragility-robustness-sensitivity-1d-v1`** / **`fragility-robustness-sensitivity-2d-v1`**. **`topology_mode`**: **`synthetic_er_ws`** \| **`neighbor_json_bundle`**. **`--topology neighbor_lists`** matches dense rollouts for synthetic ER/WS (parity tests). **GA budget sweep:** `robustness_ga_generations_1d_sweep`, **`--ga-budget-sweep`** → **`fragility-robustness-ga-budget-1d-v1`**. **Mechanism-design smoke:** `scripts/mechanism_design_policy_sweep.py` → **`fragility-mechanism-design-outer-v1`**. **Twin-domain composite (decoupled):** `twin_domain_rollout_artifact` / `scripts/institutional_composite_demo.py` → **`fragility-institutional-composite-v1`**.
