...
**Phase H exploration (robustness, GA budgets, mechanism design, composites):** Implemented in `fragility_engine.benchmarks.ensemble` and the CLIs indexed in [`benchmarks/README.md`](benchmarks/README.md) (`fragility_robustness_sweep.py`, `mechanism_design_policy_sweep.py`, `institutional_composite_demo.py`). Schemas include `fragility-robustness-ensemble-v1`, `fragility-robustness-sensitivity-1d-v1`, `fragility-robustness-sensitivity-2d-v1`, `fragility-robustness-ga-budget-1d-v1`, `fragility-robustness-ga-budget-2d-v1`, `fragility-robustness-ga-population-1d-v1`, `fragility-mechanism-design-outer-v1`, `fragility-institutional-composite-v1`, `fragility-institutional-composite-v2`. **`topology_mode`**: `synthetic_er_ws` \| `neighbor_json_bundle`; synthetic **`--topology neighbor_lists`** matches dense rollouts (parity tests). **Static replay viewer** loads rollout replay JSON only; sweep / composite / Pareto payloads use other contracts — [`artifacts/replay_viewer/README.md`](artifacts/replay_viewer/README.md).
