...
**Phase H exploration (robustness, GA budgets, mechanism design, composites):** Implemented in `fragility_engine.benchmarks.ensemble` and the CLIs indexed in [`benchmarks/README.md`](benchmarks/README.md) (`fragility_robustness_sweep.py`, `mechanism_design_policy_sweep.py`, `institutional_composite_demo.py`). Schemas include `fragility-robustness-ensemble-v1`, `fragility-robustness-sensitivity-1d-v1`, `fragility-robustness-sensitivity-2d-v1`, `fragility-robustness-ga-budget-1d-v1`, `fragility-robustness-ga-budget-2d-v1`, `fragility-robustness-ga-population-1d-v1`, `fragility-mechanism-design-outer-v1`, `fragility-institutional-composite-v1`, `fragility-institutional-composite-v2`, `fragility-institutional-composite-v3`. **Benchmark manifest** `benchmark-manifest-v2` (`build_benchmark_manifest`) adds provenance, per-bundle topology hints, golden-metrics digest, and pointers to **2-D minimization hypervolume** (`fragility_engine.benchmarks.hypervolume`) and **mechanical explanation DAG** (`explanation-dag-v1`, `scripts/export_explanation_dag.py`). **`topology_mode`**: `synthetic_er_ws` \| `neighbor_json_bundle`; synthetic **`--topology neighbor_lists`** matches dense rollouts (parity tests). **Static replay viewer** loads rollout replay JSON only; sweep / composite / Pareto payloads use other contracts — [`artifacts/replay_viewer/README.md`](artifacts/replay_viewer/README.md).
...
**Mechanical explanation DAG (shipped):** schema **`explanation-dag-v1`** — `fragility_engine.explain.explanation_dag` + `scripts/export_explanation_dag.py` (from counterfactual JSON or minimization report JSON; optional **`--minimization-report-out`** on `export_minimized_replay.py`).
...
**Status:** [`parallel_rollouts`](src/fragility_engine/parallel_rollouts.py) (`thread_pool_map_ordered`, `process_pool_map_ordered`), **`eval_workers`** + **`eval_pool`** (`threads` \| `processes`) on [`monte_carlo_search` / `genetic_search` / `genetic_vector_search`](src/fragility_engine/adversary/search.py), CLI **`--eval-workers`** / **`--eval-pool`** on MC / GA / co-evolution / **`benchmark_rollout`**, **[`run_benchmark_suite.py --bench-search`](scripts/run_benchmark_suite.py)**; picklable bundle rollout via [`rollout_bundle_with_genome`](src/fragility_engine/benchmarks/suite.py); isolated-template helpers in [`coevolution/thread_safe_template.py`](src/fragility_engine/coevolution/thread_safe_template.py). Optional Numba resource-cascade backend + CI **`numba-parity`** job and env **`FRAGILITY_RESOURCE_CASCADE_BACKEND`** — see [`docs/phase_k_acceleration.md`](docs/phase_k_acceleration.md).
...
The next adopted work is **not** more toy domains. Phases **Q → R → S** shipped on `main` at **v0.6.0** (memos under `docs/phase_q_*.md`, etc.).

### Phase Q — Research artifact polish (**shipped**)

**Status:** shipped — v0.6.0 README cite block, whitepaper honesty pass, `__version__` aligned with `pyproject.toml`.

**Normative memo:** [`docs/phase_q_research_artifact.md`](docs/phase_q_research_artifact.md).

**Exit criteria:**

- [x] README what-is / is-not matches these non-goals; domains labeled toy oracles.
- [x] Whitepaper intro uses counterfactual / attribution language, not “exactly why” causality.
- [x] Cite section: Zenodo DOI, BibTeX, certificate command, BYOW link.
- [x] Version fields consistent; `RELEASING.md` states the policy.
- [x] Phase Q memo linked from docs index + roadmap.

### Phase R — BYOW toolkit surface (**shipped**)

**Status:** shipped — `fragility` CLI (`console_scripts`), `fragility_engine.byow`, tutorial worlds `capacity-pool` + `token-bucket`, CI on push/PR.

**Normative memo:** [`docs/phase_r_byow_toolkit.md`](docs/phase_r_byow_toolkit.md).

**Exit criteria:**

- [x] Documented CLI entry points work on the capacity-pool example without private script imports.
- [x] BYOW surface listed in `docs/REFERENCE.md`; pytest for CLI + determinism helper.
- [x] Second generic `examples/` world + cookbook section.
- [x] README leads with BYOW → one-day worth-it test → toys as oracles.
- [x] Seven frozen bundles still validate.

### Phase S — Falsification harness (**shipped**)

**Status:** shipped — `fragility_engine.falsify`, `fragility falsify search`, `ranked-store` example, [`docs/FALSIFICATION_HARNESS.md`](docs/FALSIFICATION_HARNESS.md).

**Normative memo:** [`docs/phase_s_falsification_harness.md`](docs/phase_s_falsification_harness.md).

**Exit criteria:**

- [x] Falsification contract documented; reference adapter + determinism tests.
- [x] One pinned-seed example: search → predicate collapse → minimize → replay JSON.
- [x] Replay meta marks falsification mode; charter goldens unchanged.
- [x] `SCALE_AND_LIMITS.md` states reset cost + presence-not-absence.
