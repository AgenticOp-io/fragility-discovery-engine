...
PENTA = Path(__file__).resolve().parents[1] / "artifacts" / "composite_demo" / "sample_penta_composite.json"
...
def test_sample_penta_composite_schema_and_narration() -> None:
    if not PENTA.is_file():
        pytest.skip("bundled penta composite missing")
    data = json.loads(PENTA.read_text(encoding="utf-8"))
    assert data["schema"] == "fragility-institutional-composite-v4"
    for branch in (
        "aggregate",
        "network",
        "resource_cascade",
        ...
        assert branch in data
    text = narrate_frozen_artifact(data, source=str(PENTA))
    assert "institutional composite" in text
    assert "liquidity_ladder" in text
