...
def test_composite_viewer_presets_and_bundled_samples() -> None:
    ...
    twin = json.loads((COMPOSITE_DEMO / "sample_twin_composite.json").read_text(encoding="utf-8"))
    assert twin["schema"] == "fragility-institutional-composite-v1"
    assert "network" in twin and "resource_cascade" in twin
