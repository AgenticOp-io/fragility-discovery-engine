<#
.SYNOPSIS
  Push latest gce scripts to a GCE VM and run git pull + full ci_local parity.
...
.PARAMETER Project
  Optional gcloud project (or FRAGILITY_GCE_PROJECT).
#>
param(
...
if (-not $Instance -or -not $Zone) {
  throw "Missing instance/zone. Set FRAGILITY_GCE_INSTANCE and FRAGILITY_GCE_ZONE or pass -Instance and -Zone."
}
...
if ($hasGit -ne "yes") {
  Write-Host "==> no git clone - run: powershell -File scripts/gce_bootstrap_git.ps1"
  throw "VM is not bootstrapped for git. Run gce_bootstrap_git.ps1 once."
}

Write-Host "==> scp gce_configure_git_ssh.sh (zone=$Zone)"
Copy-GceShellScriptToVm -LocalPath $ConfigureLocal -Instance $Instance -Zone $Zone -RemoteName "gce_configure_git_ssh.sh"

Write-Host "==> scp gce_git_auth.sh"
Copy-GceShellScriptToVm -LocalPath $AuthLocal -Instance $Instance -Zone $Zone -RemoteName "gce_git_auth.sh"

Write-Host "==> scp gce_pull_and_test.sh"
...
Write-Host "==> gcloud compute ssh $Instance"
