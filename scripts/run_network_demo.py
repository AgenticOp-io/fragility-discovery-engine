...
def main() -> None:
    ...
    n = int(args.nodes)
    graph = ContagionGraph.erdos_renyi(n, p=float(args.er_p), seed=int(args.graph_seed))
    ...
    if args.export_replay is not None:
        replay["meta"] = {
            ...
            "topology": {"kind": "erdos_renyi", "nodes": n, "p": float(args.er_p), "seed": int(args.graph_seed)},
            "generations": int(args.generations),
            "population_size": int(args.population_size),
        }
