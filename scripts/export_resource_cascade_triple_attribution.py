"""Emit attribution-merge-v1 for ResourceCascadeWorld: three single-intervention branches (shared baseline)."""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Merge three resource-cascade counterfactual bundles (remove_steps, initial_overload_shift, "
            "cascade_coupling_shift) that share the same baseline rollout."
        ),
    )
    ap.add_argument("--out", type=Path, default=Path("resource_cascade_triple_attribution_merge.json"))
    ...
    ap.add_argument("--variant-initial-overload", type=float, default=0.12)
    ap.add_argument("--variant-cascade-coupling", type=float, default=None)
    ...
    if args.variant_cascade_coupling is None:
        raise SystemExit("--variant-cascade-coupling required.")
    ...
    bundle_coupling, _, _ = counterfactual_resource_cascade_cascade_coupling_shift_with_rollouts(
        genome,
        template,
    ...
    merged = merge_heterogeneous_counterfactuals(
        [bundle_remove, bundle_overload, bundle_coupling],
        strict_baseline=True,
    )
    merged["meta"] = {
        "cli": "export_resource_cascade_triple_attribution",
        ...
        "variant_cascade_coupling_branch": float(args.variant_cascade_coupling),
        "baseline_cascade_coupling_template": float(template.cascade_coupling),
        "rollout_seed": int(args.seed),
