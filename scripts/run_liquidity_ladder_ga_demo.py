"""Phase N — tiny GA + minimization on LiquidityLadderWorld (same schedule encoding as other domains)."""
...
from fragility_engine.coevolution.thread_safe_template import thread_safe_liquidity_ladder_clone
...
def main() -> None:
    p = argparse.ArgumentParser(description="GA adversary on LiquidityLadderWorld (Phase N fourth domain).")
    ...
    p.add_argument("--seed", type=int, default=809)
    p.add_argument("--initial-margin", type=float, default=0.06, help="Reset margin utilization (non-negative).")
    p.add_argument(
        "--eval-workers",
        type=int,
        default=1,
    ...
    template = LiquidityLadderWorld(population=default_stablecoin_population(), max_steps=36)
    horizon = 18
    ...
    common_meta = {
        ...
        "cli": "run_liquidity_ladder_ga_demo",
